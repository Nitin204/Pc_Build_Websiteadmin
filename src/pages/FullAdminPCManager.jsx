...
//                                                             <select 
//                                                                 className={`w-full p-2 rounded text-[11px] appearance-none focus:border-red-600 outline-none pr-8 cursor-pointer ${cardBg} ${border} ${text}`}
...
//                                 <img src={build.preview} className={`w-20 h-20 object-contain rounded-lg p-2 ${isDark ? 'bg-[#0a0a0a]' : 'bg-gray-100'} ${border}`} alt="Build" />
...
//                                         <button onClick={() => { setIsEditing(build.id); setPcBuild(build); }} className={`flex-1 hover:bg-red-600 py-2 rounded text-[10px] uppercase font-bold transition-all ${isDark ? 'bg-[#1a1a1a] text-white' : 'bg-gray-200 text-black'}`}>Edit</button>
//                                         <button onClick={() => setSavedBuilds(savedBuilds.filter(b => b.id !== build.id))} className={`hover:bg-red-900 p-2 rounded transition-all ${isDark ? 'bg-[#1a1a1a] text-white' : 'bg-gray-200 text-black'}`}><Trash2 size={14}/></button>
//                                     </div>
//                                 </div>
//                             </div>
//                         ))}
...
const FullAdminPCManager = () => {
    ...
    const API_BASE = "http://localhost:8181/api/pcbuilds"; // backend URL
